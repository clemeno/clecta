...
//! in which `QueueId` they carry — so this is one function and not three, and a rule added
...
/// (PLAN §9) — getting it wrong by two pixels a row is a queue that drifts out of its own
/// scrollbar.
const ROW_PITCH: f32 = ROW_HEIGHT + CARET_HEIGHT;

/// What a drag in flight is doing to *this* queue.
///
/// `None` when nothing is being dragged, which is what turns the rows back into plain rows:
/// `mouse_area` reports every crossing otherwise, and three queues of rows would report a great
...
pub struct Dragging {
	...
	/// `Some` for at most one of the three queues (PLAN §7a).
	pub insertion: Option<usize>,
	/// Whether one of this queue's scroll edges is armed, and which: `true` is the header,
	/// scrolling up.
...
pub fn view<'a>(
	...
	let panel = container(
		column![
			...
			switches(id, queue),
			scrollable(rows(id, queue, held, insertion, scroll, tempos))
				...
				.height(Fill),
			edging(id, footer(id, queue), false, held, edge == Some(false)),
		]
		.spacing(4),
	)
	...
	mouse_area(panel)
		.on_exit(Message::DragOut(Zone::Queue(id)))
		.into()
}

/// The header and the footer, doubling as the queue's scroll edges while a drag is in flight
/// (PLAN §7a).
///
...
/// pixels of every queue to be useful for a second at a time. The header and the footer are
...
/// The queue's name, and the two ways a file from the browser gets into it.
///
/// The add buttons live here rather than in the files pane because there are three queues and
/// two ways in: six buttons in one header would each need a label saying which queue they
/// meant, where a button sitting *on* the queue needs none (PLAN §7a).
fn header(id: QueueId, addable: bool) -> Element<'static, Message> {
...
/// What this queue does when a player runs out of track (PLAN §7a).
///
...
/// and three of these panels share the width two players had. Sitting on the queue is what says
/// *this* queue, which is the whole point of the setting being per queue.
///
...
fn switches(id: QueueId, queue: &Queue) -> Element<'static, Message> {
	...
	column![
		row![
			...
			switch(queue.auto_play, "Auto-play")
...
fn rows<'a>(
	...
	for (index, item) in queue
		...
		.take(range.len())
	{
		let selected = queue.is_selected(index);
		...
		let body = container(
			row![
				...
				// would flick to numbers a moment later on every row of a freshly opened queue.
		...
		let area = mouse_area(body)
			...
			// noticed in the queue it is going to be played from (PLAN §14d).
		...
		// player panels: `mouse_area` reports every crossing otherwise, and a queue of rows
	...
	// `scrollable` is not a widget and cannot be entered — and dropping at the end of a queue
	// is the commonest drop there is. It is also the only target an *empty* queue has.
...
/// Everything that can be done to the selected row: take it out, move it within the queue, or
/// hand it to a neighbour — and how long the whole queue runs for.
///
/// The `←` and `→` sit at the outer edges, facing the queue they send to, so the pair either
/// side of a gap reads as one control *between* two queues rather than two controls belonging
/// to one (PLAN §7a).
fn footer(id: QueueId, queue: &Queue) -> Element<'static, Message> {
	...
	let (first, last) = (queue.first_selected(), queue.last_selected());
	...
	let send = |right: bool| {
		...
		let allowed = queue.has_selection() && id.neighbour(right).is_some();
	...
	row![
		...
		edit(
			"✕",
			queue.has_selection().then_some(Message::QueueRemove(id))
		),
		text(running_time(queue))
			.size(11)
			.width(Fill)
			.align_x(iced::Right),
		send(true),
...
///
/// The `+` is the honest part: a queue whose rows are still being measured, or that holds a
...
fn running_time(queue: &Queue) -> String {
	let count = queue.items().len();
	if count == 0 {
		return String::new();
	}

	let (total, whole) = queue.total();
	format!(
		"{count} · {}{}",
...
mod tests {
	...
	fn an_arrow_is_dead_at_the_end_it_is_pointing_at() {
		...
		// The single row of a single-row queue, which is where an off-by-one would show first.
	...
	fn a_running_time_says_when_it_is_still_counting() {
		// Arrange
		let mut queue = Queue::from_paths(vec![PathBuf::from("/m/a.mp3")]);

		// Act / Assert: an empty queue says nothing at all — a `0 · 0:00` in every empty panel
		// is three pieces of furniture saying there is nothing there.
		assert_eq!(running_time(&Queue::default()), "");

		// Measured or not, the count is right; the `+` is what changes.
		assert_eq!(running_time(&queue), "1 · 0:00+", "not measured yet");
		...
		assert_eq!(running_time(&queue), "1 · 3:35", "measured");
	}
}
