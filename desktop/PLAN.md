...
- **`settings.rs`** — a round trip, and every broken input (empty, truncated JSON, wrong
  ...
  is the same "one bad value does not take the good ones with it" rule the faders follow. And
  the queue round trip is its own test (Q55): the file stores paths per player and switches per
  drawn queue, and the zip between those two orderings — now beside the fields themselves — is
  exactly the place a swapped index would have been silent.
...
Rows are appended, never rewritten: a decision that changes gets a new row pointing back at
the old one (Q28 revisits Q14), so the log keeps the order things were learned in. The one
exception is vocabulary (Q52) — a glossary sweep may retire a word inside an old row, because
the log is read with today's glossary — and it must be total, and must never touch what a row
decided.
...
| Q52 | Whether a vocabulary sweep may rewrite the decision log's own history | **The words yes, the meaning never.** Q49's sweep already rewrote retired words inside Q20, Q22, Q26, Q39 and Q47, and this row makes that an explicit exception to the append-only rule rather than a breach of it: the log is read with today's glossary, and a retired word in an old row reads as a second concept — the exact confusion the glossary exists to kill. What append-only protects is the *decision*, what was chosen and why, and no sweep touches that. The exception carries its own rule: the sweep must be total — Q41 kept **Clear BPM edits**, a button that no longer exists, until this row's commit swept it — and a partial sweep is worse than none, because it makes the old word look deliberate | §15, Q49, CONTEXT.md |
| Q53 | Whether "a new press cancels the pending collapse" means row presses or any press | **Any press, any button, from the raw listener.** §9a had already promised it and the code kept it only for rows: a press held on a deck button across the 350 ms tick let the collapse fire under it, and the button's release then acted on one narrowed row instead of the selection. The fix is one arm in the `gestures()` listener — the same always-on listener, and the same ponytail trade, as the drag's every-release arm — publishing a message whose whole handler is `pending = None`. Pending only, deliberately twice over: the *armed* stage belongs to the very press being processed, so clearing it here would race the row's own message — and the row arms keep their own clear, because a subscription message can trail the widget's by a frame and the timer does not wait. Any button, because a right press opens the menu that acts on the clicked row — the same hazard by another door | §9a, §6, Q50 |
...
| Q60 | What Unload does, and what it asks first | **Empty the player, and ask only while the sound is on.** The button sits beside **Load…** and puts the player back to `Empty` — track, playhead, waveform and the scanning flag all go; the fader stays, because it is a mixer setting rather than track state. The pairing is Q48's: `Deck::unloaded` moves the model and the audio together, with no `Option<String>` return because dropping a source is the one thing a device cannot refuse. The scanning flag must go *here*: the `Scanned` guard drops a result for a track no longer loaded, so nothing else would ever stop the animation. The confirmation is the same rfd modal the destructive buttons use, and only for a playing player — unloading a stopped track costs one Load to undo, while cutting audible playback is the mistake a modal exists for. Auto-load cannot refill the emptied player: `advance` runs only when a track *ends* (§7a), and an unloaded player has no track to end | §7, Q48, §7a |
