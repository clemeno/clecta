...
- **Two players.** Play / pause / stop, a `M:SS / M:SS` readout, and **Load…** / **Unload**
  buttons each. Stop is a rewind, not a `Player::stop()`: the track stays loaded and plays
  again from the top (PLAN §7). Unload empties the player — asking first only when it is
  playing, since that is the one case where the button cuts sound (Q60).
...
| `known.rs` | one answer landing in every place that shows it — the trim the players read, the mark on the pane's row, the queued rows holding the file — and a job that could not say taking nothing away: a queue measurement that found nothing neither un-learns a trim nor takes a mark off a row |
